...
def test_engine_guard_day_logs_dup(fresh_db, capsys, monkeypatch):
    ...
    with models.SessionLocal() as s:
        for i in range(1, 3):
            s.add(models.Staff(id=i, full_name=f"TC{i}", role="TC", can_night=True, base_quota=26.0))
        s.commit()
    ...
    ids = [p.staff_id for p in ctx._planned if p.day == d and p.shift_code == "K" and p.position == "TD"]
    assert ids == [leader1]
    close_context(ctx)
